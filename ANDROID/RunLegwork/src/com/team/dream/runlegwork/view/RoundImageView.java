...
public class RoundImageView extends ImageView {
	...
	private void setUpShader() {
		...
		if (type == TYPE_CIRCLE) {
			...
			if (getScaleType() == ScaleType.CENTER_CROP) {
				...
				// scale = 1.0f;
				// } else {
				// scale = Math.max((float) vwidth / (float) dwidth, (float)
				// vheight
				// / (float) dheight);
				// }
	...
	protected void onDraw(Canvas canvas) {
		...
		if (type == TYPE_ROUND) {
			 ...
			 mBitmapPaint);
//			float width = mRoundRect.width();
//			float height = mRoundRect.height();
//			RectF topLeft = new RectF(0, 0, mBorderRadius, mBorderRadius);
//			RectF topRight = new RectF(width - mBorderRadius, 0, width, mBorderRadius);
//			Path path = new Path();
